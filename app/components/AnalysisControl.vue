...
<template>
  <div class="space-y-6 mb-20">
    <div class="flex justify-start px-5 mb-5">
      ...
      <button
        type="button"
        class="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg border border-amber-200 bg-amber-50 text-sm font-medium text-amber-900 hover:bg-amber-100 transition"
      ...
      <button
        type="button"
        @click="showRiskMatrixModal = true"
        class="inline-flex ml-5 px-4 py-1.5 bg-sky-100 text-sky-700 border border-sky-300 rounded-lg text-sm font-medium hover:bg-sky-200 transition flex items-center gap-1"
      >
        <span>📊</span>
      ...
      <button
        type="button"
        @click="showOutputExampleModal = true"
        class="inline-flex ml-5 px-4 py-1.5 bg-green-100 text-green-700 border border-green-300 rounded-lg text-sm font-medium hover:bg-green-200 transition flex items-center gap-1"
      >
        <span>📋</span>
    ...
    <transition name="fade-overlay">
      ...
      >
        <div
          class="relative bg-white rounded-3xl shadow-2xl max-w-5xl w-full mx-4 p-6 md:p-8"
        >
          <!-- close button -->
          <button
            type="button"
          ...
          <h2 class="text-xl md:text-2xl font-bold text-gray-900 mb-4">
            ⚠️ Chemical / material hazards analysis
          </h2>
          ...
          <div class="flex flex-col md:flex-row gap-8">
            ...
            <div class="md:w-2/5 space-y-4">
              <div
                class="rounded-2xl border border-gray-200 bg-gray-100 px-4 py-3"
              >
                <div
              ...
              <div class="rounded-2xl bg-lime-200/60 p-3 space-y-3">
                <div class="rounded-xl bg-white px-4 py-3">
                  ...
                  <ul class="text-sm text-gray-900 space-y-0.5">
                    <li
                      v-if="!processInputs || processInputs.length === 0"
                      class="italic text-gray-400"
                    >
                      No inputs detected from JSON
                    </li>
                    <li
                      v-for="(inp, idx) in processInputs"
                      :key="`in-${idx}-${inp}`"
                    >
                      • {{ inp }}
                    </li>
                ...
                <div class="rounded-xl bg-white px-4 py-3">
                  ...
                  <ul class="text-sm text-gray-900 space-y-0.5">
                    <li
                      v-if="!processOutputs || processOutputs.length === 0"
                      class="italic text-gray-400"
                    >
                      No outputs detected from JSON
                    </li>
                    <li
                      v-for="(out, idx) in processOutputs"
                      :key="`out-${idx}-${out}`"
                    >
                      • {{ out }}
                    </li>
            ...
            <div class="md:flex-1 grid sm:grid-cols-2 gap-3">
              <div
                class="rounded-xl bg-amber-50 px-4 py-3 text-sm font-semibold text-gray-900 flex items-center gap-2"
              >
                <span>🧍‍♂️☠️</span>
    ...
    <transition name="fade-overlay">
      <div
        v-if="showRiskMatrixModal"
        class="fixed inset-0 z-50 flex items-center justify-center bg-black/40"
      >
        <div
          class="bg-white rounded-2xl shadow-2xl max-w-5xl w-full mx-4 max-h-[90vh] overflow-hidden flex flex-col"
        >
          <!-- header -->
          <div
          ...
          >
            <div class="flex items-center gap-3">
              <div
                class="w-10 h-10 rounded-xl bg-sky-100 text-sky-700 flex items-center justify-center text-2xl"
              >
                📊
              </div>
              <div>
          ...
          <div class="flex flex-1 overflow-hidden bg-white">
            ...
            <div class="flex-1 overflow-auto p-6 px-20">
              <div class="mb-4">
                <div
                  class="rounded-2xl border border-gray-200 bg-gray-100 px-4 py-3 w-60 mb-3"
                >
                  <div
                    class="text-xs font-semibold text-gray-600 uppercase tracking-wide"
                  >
                ...
                <p class="text-xs text-gray-500 ml-5">
                  Use this as a visual guide when assigning risk levels to each
                  HAZOP deviation.
                </p>
              </div>

              <div
                class="inline-block border border-gray-200 rounded-xl bg-white"
              >
                <table class="text-xs">
                  <thead>
                    <tr>
                      <th
                        class="bg-orange-100 text-gray-900 px-3 py-2 align-bottom"
                        rowspan="2"
                      >
                        Severity
                      </th>
                      <th
                        class="bg-orange-100 text-gray-900 px-8 py-1 text-center border-l border-gray-200"
                  ...
                  <tbody>
                    ...
                    >
                      ...
                      >
                        <div
                          class="mx-auto w-10 h-10 rounded-md flex items-center justify-center text-sm font-semibold"
            ...
            <div class="w-72 border-l border-gray-100 bg-white flex flex-col">
              ...
              >
                ...
                <span
                  ...
                  :class="showRiskSummary ? 'rotate-180' : ''"
                >
                  ▼
                </span>
              </button>
              ...
              <div
                ...
                class="flex-1 overflow-auto px-4 py-3 space-y-2 text-xs"
              >
                <div
          ...
          <!-- footer -->
          <div
            class="px-6 py-3 border-t border-gray-100 bg-white flex justify-end"
          >
            <button
              type="button"
      ...
      >
        ...
        >
          ...
          >
            <div>
              <h2 class="text-base font-semibold text-gray-900">
                Output analysis table – example
              </h2>
              <p class="text-xs text-gray-500">
                Example of one HAZOP deviation row showing risk before and after
          ...
          <div class="flex-1 overflow-auto p-6">
            <div
              class="rounded-2xl border border-gray-200 bg-yellow-50 px-4 py-3 mb-4 inline-block"
            >
              <div
                class="text-xs font-semibold text-gray-600 uppercase tracking-wide"
              >
                Table for HAZOP analysis
              </div>
            </div>

            <div class="overflow-x-auto">
              <table
                class="min-w-max text-xs text-left border border-gray-100 rounded-xl bg-white"
              >
                <thead>
                  <tr class="bg-yellow-50 border-b border-gray-100">
                    <th
                      v-for="head in outputTableHeaders"
                      :key="head"
                      class="px-3 py-5 align-top text-[11px] text-gray-800 border-r last:border-r-0 border-gray-200 whitespace-normal break-words max-w-[125px]"
                ...
                <tbody>
                  <tr
                    v-for="(row, rIdx) in outputExampleRows"
                    :key="rIdx"
                    class="border-t border-gray-200"
                  >
                    <td
