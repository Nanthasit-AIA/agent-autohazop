...
// Two services: P&ID extraction is its own deployable container, HAZOP is the monolith.
const {
...
// tunnel or a single App Service) serves the SPA and both backends from one host.
...
// ----------------- Socket.IO -----------------
// autoConnect is off on purpose. The HAZOP backend is not always deployed, so
// on a hosted build this would dial the wrong host on every page load - blocked
// as mixed content, then retried forever. Nothing in the extract flow needs the
// socket, so it is only dialled when analysis starts.
