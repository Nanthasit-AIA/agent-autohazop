---
name: book-safety-security-review-process-industries
description: Use source-specific working knowledge from Safety and Security Review for Process Industries for AutoHAZOP, HAZOP/PHA, LOPA, SIS, QRA, relief/consequence, reliability, PSM, incident-learning, alarm, or process-safety review when a node, deviation, safeguard, recommendation, or user question matches these tags: hazop, pha, what-if, sva, security-vulnerability, review-method. Use to improve scenario framing, missing-basis detection, evidence-traceable recommendations, and specialist handoff. Read references/wiki.md, references/field-manual.md, and references/evidence-map.md before substantive use.
---

# Safety and Security Review for Process Industries Book Skill

Use this skill to apply the `safety-security-review-process-industries` book wiki as a controlled AutoHAZOP working method. This skill is intentionally fail-closed: if a value, table, criterion, equation, example, or plant-specific fact is not encoded in the wiki/evidence map or supplied by project data, mark it as missing basis instead of guessing.
...
5. Route to shared skill `hazop-hazan-study-leader` and secondary skills `process-safety-security-review` when the decision needs a specialist workflow.
...
- Top evidence signals: HAZOP/PHA:1021, Security review:163, SIS/SIL:68, PSM/MOC/documentation:63
...
- Basis: `safety-security-review-process-industries`, matched evidence signal, source role, and confidence tier.
