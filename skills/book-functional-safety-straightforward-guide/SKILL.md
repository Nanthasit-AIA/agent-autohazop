---
name: book-functional-safety-straightforward-guide
description: Use source-specific working knowledge from Functional Safety - IEC 61508 and Related Standards for AutoHAZOP, HAZOP/PHA, LOPA, SIS, QRA, relief/consequence, reliability, PSM, incident-learning, alarm, or process-safety review when a node, deviation, safeguard, recommendation, or user question matches these tags: functional-safety, iec-61508, sil, safety-lifecycle, sif. Use to improve scenario framing, missing-basis detection, evidence-traceable recommendations, and specialist handoff. Read references/wiki.md, references/field-manual.md, and references/evidence-map.md before substantive use.
---

# Functional Safety - IEC 61508 and Related Standards Book Skill

Use this skill to apply the `functional-safety-straightforward-guide` book wiki as a controlled AutoHAZOP working method. This skill is intentionally fail-closed: if a value, table, criterion, equation, example, or plant-specific fact is not encoded in the wiki/evidence map or supplied by project data, mark it as missing basis instead of guessing.
...
5. Route to shared skill `sis-sil-verification-reliability` when the decision needs a specialist workflow.
...
- Top evidence signals: SIS/SIL:836, Reliability data:267, QRA/risk criteria:99, Incident/human factors:47
...
- Basis: `functional-safety-straightforward-guide`, matched evidence signal, source role, and confidence tier.
