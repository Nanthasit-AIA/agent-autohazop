---
name: book-sis-verification-probabilistic-calculation
description: Use source-specific working knowledge from SIS Verification - Probabilistic Calculation for AutoHAZOP, HAZOP/PHA, LOPA, SIS, QRA, relief/consequence, reliability, PSM, incident-learning, alarm, or process-safety review when a node, deviation, safeguard, recommendation, or user question matches these tags: sis, pfdavg, pfh, proof-test, common-cause, probabilistic-calculation. Use to improve scenario framing, missing-basis detection, evidence-traceable recommendations, and specialist handoff. Read references/wiki.md, references/field-manual.md, and references/evidence-map.md before substantive use.
---

# SIS Verification - Probabilistic Calculation Book Skill

Use this skill to apply the `sis-verification-probabilistic-calculation` book wiki as a controlled AutoHAZOP working method. This skill is intentionally fail-closed: if a value, table, criterion, equation, example, or plant-specific fact is not encoded in the wiki/evidence map or supplied by project data, mark it as missing basis instead of guessing.
...
- Top evidence signals: SIS/SIL:1376, Reliability data:456, QRA/risk criteria:163, LOPA/IPL:18
...
- Basis: `sis-verification-probabilistic-calculation`, matched evidence signal, source role, and confidence tier.
