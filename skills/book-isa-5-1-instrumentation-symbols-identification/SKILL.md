---
name: book-isa-5-1-instrumentation-symbols-identification
description: Use source-specific working knowledge from ISA 5.1 Instrumentation Symbols and Identification for AutoHAZOP, HAZOP/PHA, LOPA, SIS, relief/consequence, reliability, PSM, operations, maintenance, P&ID/graph extraction, or process-safety review when a node, deviation, cause, safeguard, recommendation, or user question matches these tags: isa-5-1, instrument-tag, loop, p&id-symbol, signal, control. Use to sharpen cause credibility, consequence paths, safeguard/IPL classification, missing-basis detection, and verifiable recommendations. Read references/wiki.md, references/field-manual.md, and references/evidence-map.md before substantive use.
---

# ISA 5.1 Instrumentation Symbols and Identification Book Skill

Use this skill as a controlled AutoHAZOP working method for `isa-5-1-instrumentation-symbols-identification`. This is intentionally fail-closed: if a value, table, equation, clause, acceptance criterion, inspection interval, proof-test basis, or project fact is not encoded in the wiki/evidence map or supplied by project data, mark it as missing basis instead of guessing.
...
- Primary topic wiki: `instrument-symbols-identification`
...
- Core decision lens: Use ISA instrument tag/symbol conventions to improve P&ID extraction, instrument function classification, and loop/control/safeguard interpretation.
...
- Basis: `isa-5-1-instrumentation-symbols-identification`, matched tag/domain, source role, and confidence tier.
