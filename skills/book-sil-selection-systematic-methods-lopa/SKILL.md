---
name: book-sil-selection-systematic-methods-lopa
description: Use source-specific working knowledge from SIL Selection - Systematic Methods Including LOPA for AutoHAZOP, HAZOP/PHA, LOPA, SIS, QRA, relief/consequence, reliability, PSM, incident-learning, alarm, or process-safety review when a node, deviation, safeguard, recommendation, or user question matches these tags: sil-selection, lopa, sif, tolerable-risk, probability, fault-tree. Use to improve scenario framing, missing-basis detection, evidence-traceable recommendations, and specialist handoff. Read references/wiki.md, references/field-manual.md, and references/evidence-map.md before substantive use.
---

# SIL Selection - Systematic Methods Including LOPA Book Skill

Use this skill to apply the `sil-selection-systematic-methods-lopa` book wiki as a controlled AutoHAZOP working method. This skill is intentionally fail-closed: if a value, table, criterion, equation, example, or plant-specific fact is not encoded in the wiki/evidence map or supplied by project data, mark it as missing basis instead of guessing.
...
5. Route to shared skill `lopa-iel-conditional-modifier` and secondary skills `sis-sil-verification-reliability`, `risk-criteria-qra` when the decision needs a specialist workflow.
...
- Top evidence signals: SIS/SIL:955, LOPA/IPL:369, QRA/risk criteria:326, Consequence analysis:228
...
- Basis: `sil-selection-systematic-methods-lopa`, matched evidence signal, source role, and confidence tier.
