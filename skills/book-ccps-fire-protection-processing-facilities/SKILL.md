---
name: book-ccps-fire-protection-processing-facilities
description: Use source-specific working knowledge from CCPS Fire Protection in Processing Facilities for AutoHAZOP, HAZOP/PHA, LOPA, SIS, QRA, relief/consequence, reliability, PSM, incident-learning, alarm, or process-safety review when a node, deviation, safeguard, recommendation, or user question matches these tags: fire-protection, fire-hazard-analysis, fire-risk, emergency-response, inspection-testing-maintenance. Use to improve scenario framing, missing-basis detection, evidence-traceable recommendations, and specialist handoff. Read references/wiki.md, references/field-manual.md, and references/evidence-map.md before substantive use.
---

# CCPS Fire Protection in Processing Facilities Book Skill

Use this skill to apply the `ccps-fire-protection-processing-facilities` book wiki as a controlled AutoHAZOP working method. This skill is intentionally fail-closed: if a value, table, criterion, equation, example, or plant-specific fact is not encoded in the wiki/evidence map or supplied by project data, mark it as missing basis instead of guessing.
...
5. Route to shared skill `relief-effluent-fire-explosion-consequence` and secondary skills `inherently-safer-siting-layout` when the decision needs a specialist workflow.
...
- Top evidence signals: Fire/explosion:970, Consequence analysis:382, Inherent safety/siting:215, PSM/MOC/documentation:93
...
- Basis: `ccps-fire-protection-processing-facilities`, matched evidence signal, source role, and confidence tier.
