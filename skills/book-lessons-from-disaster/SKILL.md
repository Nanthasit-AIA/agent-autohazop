---
name: book-lessons-from-disaster
description: Use source-specific working knowledge from Lessons from Disaster for AutoHAZOP, HAZOP/PHA, LOPA, SIS, QRA, relief/consequence, reliability, PSM, incident-learning, alarm, or process-safety review when a node, deviation, safeguard, recommendation, or user question matches these tags: organizational-memory, recurring-accidents, incident-learning, management-system. Use to improve scenario framing, missing-basis detection, evidence-traceable recommendations, and specialist handoff. Read references/wiki.md, references/field-manual.md, and references/evidence-map.md before substantive use.
---

# Lessons from Disaster Book Skill

Use this skill to apply the `lessons-from-disaster` book wiki as a controlled AutoHAZOP working method. This skill is intentionally fail-closed: if a value, table, criterion, equation, example, or plant-specific fact is not encoded in the wiki/evidence map or supplied by project data, mark it as missing basis instead of guessing.
...
- Top evidence signals: Incident/human factors:184, Fire/explosion:98, Consequence analysis:43, HAZOP/PHA:31
...
- Basis: `lessons-from-disaster`, matched evidence signal, source role, and confidence tier.
