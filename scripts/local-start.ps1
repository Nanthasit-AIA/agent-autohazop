...
Start-Svc "extraction :8000" $py "app.py" (Join-Path $root "services\pid-extract") @{
    ...
    LITELLM_MODELS = $(if ($vals["LITELLM_MODELS"]) { $vals["LITELLM_MODELS"] } else { $Model })
    OPENAI_API_KEY = $vals["OPENAI_API_KEY"]; OPENAI_MODELS = $vals["OPENAI_MODELS"]
...
if (Test-Path $hazopPy) {
    Start-Svc "HAZOP :5000" $hazopPy "app.py" (Join-Path $root "backend") @{
        PYTHONUNBUFFERED = "1"
        OPENAI_API_KEY = $vals["OPENAI_API_KEY"]; OPENAI_MODELS = $vals["OPENAI_MODELS"]
        LITELLM_BASE_URL = $baseUrl; LITELLM_API_KEY = $vals["LITELLM_API_KEY"]; LITELLM_MODELS = $vals["LITELLM_MODELS"]
